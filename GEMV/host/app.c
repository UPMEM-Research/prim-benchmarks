...
int main(int argc, char **argv) {
	...
	//di added for the log of the DPUs
	...
	DPU_FOREACH(dpu_set, dpu, i) {
		...
		rows_per_dpu = chunks;
		printf("1 row per dpu and chunck = %d\n", rows_per_dpu);
		uint32_t rest_rows = m_size % nr_of_dpus;
		printf("2 rest_rows = %d\n", rest_rows);

		// printf("Sex number of i : %d \n", i );
		if (i < rest_rows){
			rows_per_dpu++;
			printf("3 rows per dpu = %d\n", rows_per_dpu);
		}
		if (rest_rows > 0) {
			if (i >= rest_rows){
				prev_rows_dpu = rest_rows * (chunks + 1) + (i - rest_rows) * chunks;
				printf("4 prev rows dpu = %d\n", prev_rows_dpu);
			}
			else {
				prev_rows_dpu = i * (chunks + 1);
				printf("5 prev rows dpu = %d\n", prev_rows_dpu);}
		} else {
			prev_rows_dpu = i * chunks;
			printf("6 prev rows dpu = %d\n", prev_rows_dpu);
		}

		// Keep max rows for parallel transfers
		uint32_t rows_per_dpu_pad = rows_per_dpu;
		printf("7 rows_per_dpu_pad = %d\n", rows_per_dpu_pad);
		if (rows_per_dpu_pad % 2 == 1){
			rows_per_dpu_pad++;
			printf("8 rows_per_dpu_pad = %d\n", rows_per_dpu_pad);
		} // 4-byte elements
		if (rows_per_dpu_pad > max_rows_per_dpu){
			max_rows_per_dpu = rows_per_dpu_pad;
			printf("9 max_rows_per_dpu = %d\n", max_rows_per_dpu);
		}

		dpu_info[i].rows_per_dpu = rows_per_dpu;
		dpu_info[i].rows_per_dpu_pad = rows_per_dpu_pad;
		dpu_info[i].prev_rows_dpu = prev_rows_dpu;
		printf("@@@ setting dpu_info[%d].rows_per_dpu = %d\n", i, rows_per_dpu);
		printf("@@@ setting dpu_info[%d].rows_per_dpu_pad = %d\n", i, rows_per_dpu_pad);
		printf("@@@ setting dpu_info[%d].prev_rows_dpu = %d\n", i, prev_rows_dpu);

		// Copy input arguments to DPU
		input_args[i].n_size = n_size;
		input_args[i].n_size_pad = n_size_pad;
		input_args[i].nr_rows = rows_per_dpu;
		printf("### setting input_args[%d].n_size = %d\n", i, n_size);
		printf("### setting input_args[%d].n_size_pad = %d\n", i, n_size_pad);
		printf("### setting input_args[%d].nr_rows = %d\n\n", i, rows_per_dpu);
	}
