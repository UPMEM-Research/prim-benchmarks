...
// 그룹화 함수 (Hash 알고리즘)
GroupedData groupByHash(Matrix *matrix, int groupByColumn) {
    GroupedData groupedData;
    groupedData.groupByColumn = groupByColumn;
    groupedData.groupCount = 0;

    int hashSize = MAX_ROWS; // Hash 테이블 크기

    // Hash 테이블 초기화
    Group hashTable[hashSize];
    for (int i = 0; i < hashSize; i++) {
        hashTable[i].groupCount = 0;
    }

    for (int i = 0; i < matrix->rows; i++) {
        bool foundGroup = false;
        double groupValue = matrix->data[i][groupByColumn];
        int hashIndex = (int)groupValue % hashSize;

        for (int j = 0; j < hashTable[hashIndex].groupCount; j++) {
            int groupIndex = hashTable[hashIndex].groupIndices[0];
            double firstGroupValue = matrix->data[groupIndex][groupByColumn];

            if (isEqual(groupValue, firstGroupValue)) {
                // 기존 그룹에 추가
                hashTable[hashIndex].groupIndices[hashTable[hashIndex].groupCount] = i;
                hashTable[hashIndex].groupCount++;
                foundGroup = true;
                break;
            }
        }
        ...
        if (!foundGroup) {
            // 새로운 그룹 생성
            hashTable[hashIndex].groupIndices[0] = i;
            hashTable[hashIndex].groupCount = 1;
        }
    }

    // Hash 테이블에서 그룹화된 데이터를 GroupedData 구조체로 옮기기
    for (int i = 0; i < hashSize; i++) {
        if (hashTable[i].groupCount > 0) {
            groupedData.groups[groupedData.groupCount] = hashTable[i];
...
int main() {
    ...
    GroupedData groupedData = groupByHash(&matrix, groupByColumn);

    printf("Grouped Data (Hash):\n");
