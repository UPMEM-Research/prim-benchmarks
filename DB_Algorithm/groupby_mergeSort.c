...
int compareRows(const void *a, const void *b) {
    ...
    int row1 = *((int *)b);
    int row2 = *((int *)a);
    double value1 = matrix->data[row1][matrix->rows];
    double value2 = matrix->data[row2][matrix->rows];
...
GroupedData groupBySortMerge(Matrix *matrix, int groupByColumn) {
    ...
    for (int i = 0; i < matrix->rows; i++) {
        ...
        double groupValue = matrix->data[rowIndex][groupByColumn];
...
int main() {
    ...
    GroupedData groupedData = groupBySortMerge(&matrix, groupByColumn);

    printf("Grouped Data (Sort-Merge):\n");
